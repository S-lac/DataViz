...
Doves <- bird_baths %>% 
  ...
  # Overrride grouping : 
  group_by(bioregions,urban_rural) %>%
  # Slice the top 5 loss : 
  # slice_max(diff_total_bird_count,n=5,with_ties = FALSE) %>%
  # Filter only honeybeards
  filter(grepl('Dove',bird_type))


ggplot(data=Doves,aes(x=total_bird_count,y=bird_type,color=urban_rural))+
  ggtitle('Variations in Dove species bathing occurence between summer and winter \nby bioregion in Australia and Area type (rural/urban).')+
  geom_vline(xintercept = 0, color='black') +
  geom_point(size=4)+
  geom_segment(aes(x=0,y=bird_type,xend=diff_total_bird_count-0.25,yend=bird_type))+
  facet_grid(urban_rural~bioregions)+
  labs(x='Difference in absolute occurence (winter - summer)',y='Dove species')+
  theme_bw()
